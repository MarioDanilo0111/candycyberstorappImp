...
/* order_data:
 * index signature that can have any number of props
 *  with string keys,
 *  the values associated with these akeys must be of type `number`.
 */
type CartItem = {
  [order_data: string]: number;
  id: number;
  price: number;
  quantity: number;
};
    ...
    setCartItems((currItems) => {
      if (
        ...
        return currItems.map((item) => {
          if (item.id === id) {
            return { ...item, price, quantity: item.quantity + 1 };
          } else {
            return item;
          }
        });
